...
class SyncUITests: BaseTestCase {
    ...
    private func verifyFxASigninScreen() {
        waitForExistence(app.navigationBars["Sync and Save Data"], timeout: 30)
        waitForExistence(app.webViews.textFields["Enter your email"], timeout: 10)
        XCTAssertTrue(app.webViews.textFields["Enter your email"].exists)
        ...
        XCTAssertTrue(app.webViews.buttons["Sign up or sign in"].exists)
    }
    ...
    func testTypeOnGivenFields() {
        ...
        navigator.nowAt(NewTabScreen)
        navigator.goto(FxASigninScreen)
        waitForExistence(app.navigationBars["Sync and Save Data"], timeout: 60)

        // Tap Sign in without any value in email Password focus on Email
        waitForExistence(app.webViews.buttons["Sign up or sign in"], timeout: 20)
    ...
    func testCreateAnAccountLink() {
        ...
        waitForExistence(app.webViews.textFields["Enter your email"], timeout: 40)
    ...
    func testQRPairing() {
        ...
        waitForExistence(app.navigationBars["Sync and Save Data"], timeout: 5)
        app.navigationBars["Sync and Save Data"].buttons["Close"].tap()
