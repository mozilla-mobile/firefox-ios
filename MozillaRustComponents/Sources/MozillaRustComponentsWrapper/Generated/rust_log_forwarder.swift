...
fileprivate struct UniffiCallbackInterfaceAppServicesLogger {
    ...
    static let vtable: UniffiVTableCallbackInterfaceAppServicesLogger = UniffiVTableCallbackInterfaceAppServicesLogger(
        uniffiFree: { (uniffiHandle: UInt64) -> () in
            do {
                try FfiConverterCallbackInterfaceAppServicesLogger.handleMap.remove(handle: uniffiHandle)
            } catch {
        ...
        uniffiClone: { (uniffiHandle: UInt64) -> UInt64 in
            do {
                return try FfiConverterCallbackInterfaceAppServicesLogger.handleMap.clone(handle: uniffiHandle)
            } catch {
                fatalError("Uniffi callback interface AppServicesLogger: handle missing in uniffiClone")
            }
        },
        log: { (
            uniffiHandle: UInt64,
    ...
    nonisolated(unsafe) static let vtablePtr: UnsafePointer<UniffiVTableCallbackInterfaceAppServicesLogger> = {
        let ptr = UnsafeMutablePointer<UniffiVTableCallbackInterfaceAppServicesLogger>.allocate(capacity: 1)
...
private func uniffiCallbackInitAppServicesLogger() {
    uniffi_rust_log_forwarder_fn_init_callback_vtable_appserviceslogger(UniffiCallbackInterfaceAppServicesLogger.vtablePtr)
}
...
public func setLogger(logger: AppServicesLogger?)  {try! rustCall() {
    ...
    uniffi_rust_log_forwarder_fn_func_set_logger(
        FfiConverterOptionCallbackInterfaceAppServicesLogger.lower(logger),uniffiCallStatus
    )
}
}
/**
...
public func setMaxLevel(level: Level)  {try! rustCall() {
    ...
    uniffi_rust_log_forwarder_fn_func_set_max_level(
        FfiConverterTypeLevel_lower(level),uniffiCallStatus
    )
}
}
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_rust_log_forwarder_checksum_func_set_logger() != 57833) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_rust_log_forwarder_checksum_func_set_max_level() != 20046) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_rust_log_forwarder_checksum_method_appserviceslogger_log() != 42605) {
        return InitializationResult.apiChecksumMismatch
    }
