...
class StorageClientTests: XCTestCase {
    func testPartialJSON() {
        let body = "0"
        let o: AnyObject? = try! JSONSerialization.jsonObject(with: body.data(using: String.Encoding.utf8)!, options: JSONSerialization.ReadingOptions.allowFragments) as AnyObject?
