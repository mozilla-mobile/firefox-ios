...
class RecordTests: XCTestCase {
    ...
    func testLivemarkMissingFields() {
        ...
        let m = (livemark as MirrorItemable).toMirrorItem(modified: Date.now())
    ...
    func testQuery() {
        ...
        let mirror = query?.toMirrorItem(modified: Date.now())
    ...
    func testBookmarks() {
        ...
        guard let item = (q as? MirrorItemable)?.toMirrorItem(modified: Date.now()) else {
        ...
        guard let pMirror = (p as? MirrorItemable)?.toMirrorItem(modified: Date.now()) else {
            XCTFail("Not mirrorable!")
            return
        }
