...
class FxAClient10 {
    ...
    func login(emailUTF8: NSData, quickStretchedPW: NSData, getKeys: Bool) -> Deferred<Result<FxALoginResponse>> {
        ...
        let URL = self.URL.URLByAppendingPathComponent(getKeys ? "/account/login?keys=true" : "/account/login")
    ...
    func keys(keyFetchToken: NSData) -> Deferred<Result<FxAKeysResponse>> {
        ...
        let timestampInSeconds: Int = Int(NSDate().timeIntervalSince1970)
        let hawkValue = hawkHelper.getAuthorizationValueFor(mutableURLRequest, at: timestampInSeconds)
