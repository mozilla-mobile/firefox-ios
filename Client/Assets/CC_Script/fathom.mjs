...
function attributesMatch(element, predicate, attrs = []) {
  ...
  for (let i = 0; i < attributes.length; i++) {
    ...
    if (
      attr &&
      ((Array.isArray(attr) && attr.some(predicate)) || predicate(attr))
    ) {
      return true;
    }
  }
...
function rgbaFromString(str) {
  ...
  if (m) {
    return [
      ...
      m[2] / 255,
      m[3] / 255,
      m[4] === undefined ? undefined : parseFloat(m[4]),
    ];
  } else {
    throw new Error(
...
function numStrides(left, right) {
  ...
  while (shouldContinue) {
    ...
    if (
      (shouldContinue = sibling && sibling !== right) &&
...
function distance(
  ...
  if (
    ...
    (elementA.DOCUMENT_POSITION_CONTAINS |
      elementA.DOCUMENT_POSITION_CONTAINED_BY)
  ) {
    ...
    for (let outerKey of this._matrix.keys()) {
      if (outerKey !== clusterA && outerKey !== clusterB) {
        newRow.set(
          outerKey,
          Math.min(
            this._cachedDistance(clusterA, outerKey),
...
class Lhs {
  ...
  static fromFirstCall(firstCall) {
    ...
    if (firstCall.method === "type") {
      ...
      throw new Error(
        "The left-hand side of a rule() must start with dom(), type(), and(), or nearest()."
      );
    }
  }
...
class DomLhs extends Lhs {
  constructor(selector) {
    ...
    if (selector === undefined) {
      throw new Error(
        "A querySelector()-style selector is required as the argument to " +
          this._callName() +
          "()."
      );
    }
...
class TypeMaxLhs extends AggregateTypeLhs {
  ...
  fnodes(ruleset) {
    ...
    return setDefault(ruleset.maxCache, this._type, function maxFnodesOfType() {
      return maxes(getSuperFnodes(), (fnode) =>
        ruleset.weightedScore(fnode.scoresSoFarFor(self._type))
      );
    });
  }
}
...
class BestClusterLhs extends AggregateTypeLhs {
  ...
  fnodes(ruleset) {
    ...
    const clustsAndSums = clusts.map((clust) => [
      clust,
...
function sideToTypeLhs(side) {
  ...
  if (!(lhs.constructor === TypeLhs)) {
    throw new Error(
      "and() and nearest() support only simple type() calls as arguments for now."
    );
...
class InwardRhs {
  ...
  _checkAtMost(fact) {
    if (fact.score !== undefined && fact.score > this._max) {
      throw new Error(
        `Score of ${fact.score} exceeds the declared atMost(${this._max}).`
      );
    }
  }
  ...
  props(callback) {
    function getSubfacts(fnode) {
      ...
      for (let subfact in subfacts) {
        if (
          !SUBFACTS.hasOwnProperty(subfact) ||
          !(SUBFACTS[subfact] & getSubfacts.possibleSubfacts)
        ) {
    ...
    return new this.constructor(
      this._calls.concat(getSubfacts),
      this._max,
      this._types
    );
  }
...
class Side {
  ...
  asLhs() {
    return this._asSide(
      Lhs.fromFirstCall(this._calls[0]),
      this._calls.slice(1)
    );
  }
...
class Fnode {
  ...
  constructor(element, ruleset) {
    if (element === undefined) {
      throw new Error(
        "Someone tried to make a fnode without specifying the element they're talking about."
      );
    }
    /**
  ...
  scoreFor(type) {
    this._computeType(type);
    return sigmoid(
      this._ruleset.weightedScore(this.scoresSoFarFor(type)) +
...
function rule(lhs, rhs, options) {
  ...
  return new (rhs instanceof OutwardRhs ? OutwardRule : InwardRule)(
    lhs,
    rhs,
    options
  );
...
class Rule {
  ...
  prerequisites(ruleset) {
    ...
    extendOrThrow(
      prereqs,
      this._typesFinalized(),
      (type) => ruleset.inwardRulesThatCouldEmit(type),
      "emits"
    );
    ...
    extendOrThrow(
      prereqs,
      this.lhs.typesMentioned(),
      (type) => ruleset.inwardRulesThatCouldAdd(type),
      "adds"
    );
  ...
  results(ruleset) {
    ...
    forEach(
      ...
      function updateFnode(leftResult) {
        ...
        const { fnode: leftFnode = leftResult, rhsTransformer = identity } =
          leftResult;
        ...
        const rightFnode = ruleset.fnodeForElement(
          fact.element || leftFnode.element
        );
...
class OutwardRule extends Rule {
  ...
  results(ruleset) {
    ...
    function justFnode(fnodeOrStruct) {
      return fnodeOrStruct instanceof Fnode
        ? fnodeOrStruct
        : fnodeOrStruct.fnode;
    }

    return this.rhs.allCallback(
      map(this.rhs.callback, map(justFnode, this.lhs.fnodes(ruleset)))
    );
  }
  ...
  constructor(rules, coeffs = [], biases = []) {
    ...
    for (let rule of rules) {
      if (rule instanceof InwardRule) {
        ...
        throw new Error(
          `This element of ruleset()'s first param wasn't a rule: ${rule}`
        );
      }
    }
  }
...
class BoundRuleset {
  ...
  _execute(rule) {
    ...
    try {
      sorted = [rule].concat(
        toposort(prereqs.keys(), (prereq) => prereqs.get(prereq))
      );
    } catch (exc) {
