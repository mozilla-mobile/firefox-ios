    ...
    static func gleanRecordEvent(category: EventCategory, method: EventMethod, object: EventObject, value: EventValue? = nil, extras: [String: Any]? = nil) {
        ...
        case (.information, .view, .homeTabBanner, .messageImpression, let extras):
            if let messageId = extras?[EventExtraKey.messageKey.rawValue] as? String {
                GleanMetrics.Messaging.messageShown.record(
                    GleanMetrics.Messaging.MessageShownExtra(messageKey: messageId)
                )
            }
        case(.action, .tap, .homeTabBanner, .messageDismissed, let extras):
            if let messageId = extras?[EventExtraKey.messageKey.rawValue] as? String {
                GleanMetrics.Messaging.messageDismissed.record(
                    GleanMetrics.Messaging.MessageDismissedExtra(messageKey: messageId)
                )
            }
        case(.action, .tap, .homeTabBanner, .messageInteracted, let extras):
            if let messageId = extras?[EventExtraKey.messageKey.rawValue] as? String,
                ...
                GleanMetrics.Messaging.messageClicked.record(
                    GleanMetrics.Messaging.MessageClickedExtra(actionUuid: actionUUID, messageKey: messageId)
                )
            }
        case(.information, .view, .homeTabBanner, .messageExpired, let extras):
            if let messageId = extras?[EventExtraKey.messageKey.rawValue] as? String {
                GleanMetrics.Messaging.messageExpired.record(
                    GleanMetrics.Messaging.MessageExpiredExtra(messageKey: messageId)
                )
            }
