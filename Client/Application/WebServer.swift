...
class WebServer {
    ...
    func start() -> Bool {
        return server.running || server.startWithOptions([GCDWebServerOption_Port: 0, GCDWebServerOption_BindToLocalhost: true, GCDWebServerOption_AutomaticallySuspendInBackground: false])
    }
    ...
    func registerMainBundleResourcesOfType(type: String, module: String) {
        for path in NSBundle.pathsForResourcesOfType(type, inDirectory: NSBundle.mainBundle().bundlePath) {
