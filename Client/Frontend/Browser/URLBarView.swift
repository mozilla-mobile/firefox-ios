...
struct URLBarViewUX {
    ...
    static func backgroundColor(withAlpha alpha: CGFloat) -> UIColor {
...
class URLBarView: UIView {
    ...
    private func commonInit() {
        backgroundColor = URLBarViewUX.backgroundColor(withAlpha: 0)
    ...
    func updateAlphaForSubviews(_ alpha: CGFloat) {
        ...
        self.backgroundColor = URLBarViewUX.backgroundColor(withAlpha: 1 - alpha)
    ...
    func updateTabCount(_ count: Int, animated: Bool = true) {
        ...
        if currentCount != countToBe {
            ...
            newTabsButton.addTarget(self, action: #selector(URLBarView.SELdidClickAddTab), for: UIControlEvents.touchUpInside)
...
extension URLBarView: TabLocationViewDelegate {
    ...
    func tabLocationViewDidTapLocation(_ tabLocationView: TabLocationView) {
        let locationText = delegate?.urlBarDisplayText(forURL: locationView.url)
...
private class CurveView: UIView {
    ...
    private func getWidthForHeight(_ height: Double) -> Double {
        return height * ASPECT_RATIO
    }

    private func drawFromTop(_ path: UIBezierPath) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        context?.saveGState()
        context?.clear(rect)
        context?.setFillColor(URLBarViewUX.backgroundColor(withAlpha: 1).cgColor)
        getPath().fill()
        leftCurvePath.fill()
