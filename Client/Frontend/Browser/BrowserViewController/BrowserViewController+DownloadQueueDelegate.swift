...
extension BrowserViewController: DownloadQueueDelegate {
        ...
        guard let downloadToast = self.downloadToast else {
            let downloadToast = DownloadToast(download: download, completion: { buttonPressed in
                ...
                if buttonPressed, !downloadQueue.isEmpty {
                    ...
                    let downloadCancelledToast = ButtonToast(labelText: .DownloadCancelledToastLabelText, imageName: "fail", textAlignment: .left)
    ...
    func downloadQueue(_ downloadQueue: DownloadQueue, didCompleteWithError error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            if error == nil {
                let downloadCompleteToast = ButtonToast(labelText: download.filename, imageName: "download", buttonText: .DownloadsButtonTitle, completion: { buttonPressed in
                ...
                let downloadFailedToast = ButtonToast(labelText: .DownloadFailedToastLabelText, imageName: "fail", textAlignment: .left)
