    ...
    init(tab: Tab) {
        [(WKUserScriptInjectionTime.atDocumentStart, mainFrameOnly: false),
         ...
         (WKUserScriptInjectionTime.atDocumentEnd, mainFrameOnly: true)].forEach { arg in
            let (position, mainFrameOnly) = arg
            let name = (mainFrameOnly ? "MainFrame" : "AllFrames") + "AtDocument" + (position == .atDocumentStart ? "Start" : "End")
            if let path = Bundle.main.path(forResource: name, ofType: "js"),
               let source = try? NSString(contentsOfFile: path, encoding: String.Encoding.utf8.rawValue) as String {
                let wrappedSource = "(function() { var SECURITY_TOKEN = '\(UserScriptManager.securityToken)'; \(source) })()"
                let userScript = WKUserScript(source: wrappedSource, injectionTime: position, forMainFrameOnly: mainFrameOnly)
