...
class BoolSetting: Setting, FeatureFlagsProtocol {
    ...
    convenience init(with featureFlagID: FeatureFlagName, titleText: NSAttributedString, defaultValue: Bool? = nil) {
    ...
    func displayBool(_ control: UISwitch) {
        if let featureFlagName = featureFlagName {
            control.isOn = featureFlags.isFeatureActive(featureFlagName)
