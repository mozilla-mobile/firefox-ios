...
class BaseTestCase: XCTestCase {
    ...
    func waitForTabsButton() {
        if iPad() {
        waitForExistence(app.buttons["TopTabsViewController.tabsButton"], timeout: 20)
        } else {
            ...
            if (XCUIDevice.shared.orientation == UIDeviceOrientation.landscapeLeft) {
                waitForExistence(app.buttons["URLBarView.tabsButton"], timeout: 20)
            } else {
                waitForExistence(app.buttons["TabToolbar.tabsButton"], timeout: 20)
            }
        }
    }
}
