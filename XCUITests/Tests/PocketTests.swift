...
class PocketTest: BaseTestCase {
    ...
    func testPocketEnabledByDefault() {
        ...
        Base.helper.waitForExistence(app.staticTexts["pocketTitle"])
        ...
        navigator.goto(NewTabScreen)
        Base.helper.waitForNoExistence(app.staticTexts["pocketTitle"])
        // Enable it again
        navigator.performAction(Action.TogglePocketInNewTab)
        navigator.goto(NewTabScreen)
        Base.helper.waitForExistence(app.staticTexts["pocketTitle"])
    ...
    func testTapOnMore() {
        ...
        navigator.goto(NewTabScreen)
        Base.helper.waitForExistence(app.buttons["More"], timeout: 5)
        app.buttons["More"].tap()
        Base.helper.waitUntilPageLoad()
        navigator.nowAt(BrowserTab)
        Base.helper.waitForExistence(app.textFields["url"], timeout: 15)
