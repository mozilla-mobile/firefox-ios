...
class NavigationTest: BaseTestCase {
    func testNavigation() {
        ...
        Base.helper.waitForValueContains(app.textFields["url"], value: "test-example.html")
        if Base.helper.iPad() {
        ...
        Base.helper.waitUntilPageLoad()
        Base.helper.waitForValueContains(app.textFields["url"], value: "test-mozilla-org.html")
        if Base.helper.iPad() {
            XCTAssertTrue(app.buttons["URLBarView.backButton"].isEnabled)
            XCTAssertFalse(app.buttons["Forward"].isEnabled)
        ...
        Base.helper.waitUntilPageLoad()
        Base.helper.waitForValueContains(app.textFields["url"], value: "test-mozilla-org")
    }
...
//        Base.helper.waitForExistence(app.navigationBars["Client.FxAContentView"], timeout: 50)
//        Base.helper.waitForExistence(app.webViews.textFields.element(boundBy: 0), timeout:50)
...
//        if isTablet {
//            Base.helper.waitForExistence(app.webViews.textFields.element(boundBy: 0), timeout: 40)
//            let email = app.webViews.textFields.element(boundBy: 0)
//            // Verify the placeholdervalues here for the textFields
//            let mailPlaceholder = "Email"
//            let defaultMailPlaceholder = email.placeholderValue!
//            XCTAssertEqual(mailPlaceholder, defaultMailPlaceholder, "The mail placeholder does not show the correct value")
//        } else {
//            Base.helper.waitForExistence(app.textFields.element(boundBy: 0), timeout: 40)
...
//        }
    ...
    func testScrollsToTopWithMultipleTabs() {
        ...
        Base.helper.waitForValueContains(app.textFields["url"], value: website_1["value"]!)
    ...
    func testLongPressLinkOptions() {
        ...
        Base.helper.waitForExistence(app.scrollViews.staticTexts[website_2["moreLinkLongPressUrl"]!])
    ...
    func testCopyLink() {
        ...
        app.buttons["Go"].tap()
        Base.helper.waitUntilPageLoad()
        Base.helper.waitForValueContains(app.textFields["url"], value: website_2["moreLinkLongPressInfo"]!)
    }
    ...
    func testLongPressOnAddressBar() {
        ...
        Base.helper.waitForExistence(app.menuItems["Select All"], timeout: 3)
    ...
    func testDownloadLink() {
        ...
        Base.helper.waitUntilPageLoad()
        Base.helper.waitForValueContains(app.textFields["url"], value: "reserved.html")
    }
    // Disable issue 5554
    /*
    func testShareLink() {
        longPressLinkOptions(optionSelected: "Share Link")
        Base.helper.waitForExistence(app.collectionViews.buttons["Copy"])
    ...
    func testCancelLongPressLinkMenu() {
        ...
        Base.helper.waitForNoExistence(app.sheets[website_2["moreLinkLongPressInfo"]!])
    ...
    func testPopUpBlocker() {
        ...
        navigator.goto(SettingsScreen)
        Base.helper.waitForExistence(app.tables["AppSettingsTableViewController.tableView"])
        ...
        Base.helper.waitForValueContains(app.textFields["url"], value: "blocker.html")
        Base.helper.waitForExistence(app.webViews.staticTexts["Blocked Element"])
    ...
    func testSSL() {
        ...
        Base.helper.waitForExistence(app.buttons["Advanced"], timeout: 10)
        app.buttons["Advanced"].tap()

        Base.helper.waitForExistence(app.links["Visit site anyway"])
        app.links["Visit site anyway"].tap()
        Base.helper.waitForExistence(app.webViews.otherElements["expired.badssl.com"], timeout: 10)
    ...
    func testWriteToChildPopupTab() {
        ...
        Base.helper.waitForExistence(app.links["link-created-by-parent"], timeout: 10)
    }
 }
