...
class FindInPageTests: BaseTestCase {
    private func openFindInPageFromMenu() {
        ...
        Base.helper.waitForExistence(app.buttons["FindInPage.find_next"], timeout: 5)
        Base.helper.waitForExistence(app.buttons["FindInPage.find_previous"], timeout: 5)
    ...
    func testFindInLargeDoc() {
        ...
        Base.helper.waitForExistence(app/*@START_MENU_TOKEN@*/.buttons["TabLocationView.pageOptionsButton"]/*[[".buttons[\"Page Options Menu\"]",".buttons[\"TabLocationView.pageOptionsButton\"]"],[[[-1,1],[-1,0]]],[0]]@END_MENU_TOKEN@*/, timeout: 15)
        ...
        Base.helper.waitForExistence(app.tables["Context Menu"].cells["menu-FindInPage"], timeout: 10)
        app.tables["Context Menu"].cells["menu-FindInPage"].tap()

        // Enter some text to start finding
        app.textFields["FindInPage.searchField"].typeText("Book")
        Base.helper.waitForExistence(app.textFields["Book"], timeout: 15)
    ...
    func testFindFromMenu() {
        ...
        Base.helper.waitForExistence(app.staticTexts["1/6"])
        ...
        Base.helper.waitForExistence(app.staticTexts["2/6"])
        XCTAssertTrue(app.staticTexts["2/6"].exists)

        nextInPageResultButton.tap()
        Base.helper.waitForExistence(app.staticTexts["3/6"])
        ...
        navigator.goto(BrowserTab)
        Base.helper.waitForNoExistence(app.textFields["Book"])
    }
    ...
    func testFindInPageTwoWordsSearchLargeDoc() {
        ...
        app.tables["Context Menu"].cells["menu-FindInPage"].tap()
        app.textFields["FindInPage.searchField"].typeText("The Book of")
        Base.helper.waitForExistence(app.textFields["The Book of"], timeout: 15)
        XCTAssertEqual(app.staticTexts["FindInPage.matchCount"].label, "1/500+", "The book word count does match")
    }
    ...
    func testFindInPageResultsPageShowHideContent() {
        ...
        Base.helper.waitForExistence(app.staticTexts["1/5"])
    ...
    func testQueryWithNoMatches() {
        ...
        Base.helper.waitForExistence(app.staticTexts["0/0"])
    ...
    func testBarDissapearsWhenReloading() {
        ...
        Base.helper.waitForNoExistence(app.textFields[""])
    ...
    func testBarDissapearsWhenOpeningTabsTray() {
        ...
        Base.helper.waitForExistence(app.collectionViews.cells["The Book of Mozilla"])
    ...
    func testFindFromSelection() {
        ...
        Base.helper.waitUntilPageLoad()
        Base.helper.waitForExistence(app.webViews.staticTexts[textToFind])
        ...
        if (app.menuItems["Find in Page"].exists) {
            ...
            Base.helper.waitForExistence(app.menuItems["Find in Page"])
            app.menuItems["Find in Page"].tap()
        }
        Base.helper.waitForExistence(app.textFields[textToFind])
