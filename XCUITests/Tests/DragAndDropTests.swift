...
class DragAndDropTests: BaseTestCase {
    ...
    func testRearrangeTabsTabTray() {
        ...
        Base.helper.waitForExistence(app.collectionViews.cells["Internet for people, not profit — Mozilla"], timeout: 10)
    ...
    func testRearrangeMoreThan3TabsTabTray() {
        ...
        if Base.helper.iPad() {
            ...
            Base.helper.waitForExistence(app.buttons["TabToolbar.tabsButton"], timeout: 10)
        }
...
class DragAndDropTestIpad: IpadOnlyTestCase {
    ...
    func testDragAndDropHomeTab() {
        ...
        Base.helper.waitUntilPageLoad()
        checkTabsOrder(dragAndDropTab: false, firstTab: homeTabName, secondTab: secondWebsite.tabName)
        Base.helper.waitForExistence(app.collectionViews.cells.element(boundBy: 1))
    ...
    func testDragAddressBarIntoSearchBox() {
        ...
        Base.helper.waitForValueContains(app.webViews.searchFields.element(boundBy: 0), value: "Search")
    ...
    func testTryDragAndDropHistoryToURLBar() {
        ...
        navigator.goto(LibraryPanel_History)
        Base.helper.waitForExistence(app.tables["History List"].cells.staticTexts[twitterTitle])
