...
class UpdateCoverSheetViewModelTests: XCTestCase {
    ...
    func testShouldNotShowCoverSheetCleanInstall() {
        ...
        let shouldShow = UpdateViewModel.shouldShow(userPrefs: prefs, currentAppVersion: currentTestAppVersion, isCleanInstall: true)
    ...
    func testShouldNotShowCoverSheetForSameVersion() {
        ...
        let shouldShow = UpdateViewModel.shouldShow(userPrefs: prefs, currentAppVersion: currentTestAppVersion, isCleanInstall: false)
    ...
    func testShouldShowCoverSheetFromNoFeatureToUpdate() {
        let olderTestAppVersion = "18.0"
        let updatedTestAppVersion = "19.0"
        ...
        let shouldShow = UpdateViewModel.shouldShow(userPrefs: prefs, currentAppVersion: updatedTestAppVersion, isCleanInstall: false)
        // Since its not clean install but we are still updating the app from a previous version that
        // already had this feature. We will save the updatedTestAppVersion to prefs and should show the update cover sheet
