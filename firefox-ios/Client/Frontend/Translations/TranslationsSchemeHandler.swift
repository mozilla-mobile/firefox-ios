...
//        respondFoo(with: data, for: url, to: task, mime: mime)
//    }
//
...
//            throw TranslationSchemeError.badURL
//        }
//        respondFoo(with: translatorWasm, for: url, to: task, mime: "application/octet-stream")
//    }
//
...
//
//        guard !id.isEmpty else { throw  TranslationSchemeError.invalidParam(name: "id", value: id) }
...
//            throw TranslationSchemeError.badURL
//        }
//        respondFoo(with: modelResponse, for: url, to: task, mime: "application/octet-stream")
//    }
//
...
//            throw TranslationSchemeError.badURL
//        }
//        respondFoo(with: modelResponse, for: url, to: task, mime: "application/json")
...
//        respondFoo(with: data, for: url, to: task, mime: "application/json")
//    }
//
//    // MARK: - Helpers
//
...
//    }
//
//    func respondFoo(with data: Data, for url: URL, to task: WKURLSchemeTask, mime: String, textEncodingName: String? = "utf-8") {
...
//            }
//
//            print("[dbg][scheme] start: dispatched route in \(String(format: "%.3f", CFAbsoluteTimeGetCurrent() - startedAt))s for \(url.absoluteString)")
//        } catch {
//            let nsError = NSError(
//                domain: "TranslationsScheme",
//                code: 1,
...
////
////        guard !id.isEmpty else { print("[dbg][scheme] models-buffer: invalid id (empty)"); throw  TranslationSchemeError.invalidParam(name: "id", value: id) }
////        
//////        guard let modelResponse = modelFetcher?.fetchModelsBuffer(withID: id) else {
//////            // TODO(Issam): Maybe throw here too. Something like model not found with pair right now just badURL
...
//        let to   = try query("to", in: components)
//
//        guard !from.isEmpty else { print("[dbg][scheme] models: invalid from (empty)"); throw  TranslationSchemeError.invalidParam(name: "from", value: from) }
//        guard !to.isEmpty else { print("[dbg][scheme] models: invalid to (empty)"); throw  TranslationSchemeError.invalidParam(name: "to", value: to) }
//        guard let modelResponse = modelFetcher?.fetchModels(from: from, to: to) else {
//            // TODO(Issam): Maybe throw here too. Something like model not found with pair right now just badURL
...
//    }
//
//    func respond(with data: Data, for url: URL, to task: WKURLSchemeTask, mime: String, textEncodingName: String? = "utf-8") {
//        print("[dbg][scheme] respond: ENTRY thread=\(Thread.current) url=\(url.absoluteString) mime=\(mime) bytes=\(data.count)")
//        
//        let headers = [
...
final class TranslationsSchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    private let attachmentQueue = DispatchQueue(label: "translations.models.attachment.serial",
                                                    qos: .userInitiated)
...
//    }
    ...
    func handleModelsBuffer(url: URL, components: URLComponents, task: WKURLSchemeTask) throws {
        ...
        attachmentQueue.async { [weak self, weak task] in
            ...
            guard let bytes = self.modelFetcher?.fetchModelsBuffer(withID: id) else {
                let err = NSError(domain: "TranslationsScheme", code: 1,
    ...
    func query(_ name: String, in components: URLComponents) throws -> String {
        guard let value = components.queryItems?.first(where: { $0.name == name })?.value,
              !value.isEmpty else {
            print("[dbg][issam][scheme] query: missing param '\(name)'")
            throw TranslationSchemeError.missingParam(name)
        }
        return value
    }
