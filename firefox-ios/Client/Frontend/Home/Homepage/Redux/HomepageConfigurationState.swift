...
struct HomepageConfigurationState: StateType, Equatable {
    ...
    /// `shouldTriggerImpression` is true only when the homepage becomes the selected tab, so impression
    /// telemetry is sent once per presentation. It is reset for every other action.
