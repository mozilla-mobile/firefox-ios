...
/// The subset of `FxAccountManager` the pairing debug setting drives, so tests
/// can inject a stub and exercise the completion path without a live FxA stack.
/// Mirrors the real method exactly so `FxAccountManager` is a direct witness.
protocol PairingAuthenticating {
...
extension FxAccountManager: PairingAuthenticating {}
...
class LaunchPairingFromURLSetting: HiddenSetting {
    /// Resolves the pairing authenticator; overridable in tests. Defaults to the
    /// account manager on the profile, which is nil outside a live FxA session.
    lazy var pairingAuthenticatorProvider: () -> PairingAuthenticating? = { [weak self] in
        self?.settings.profile?.rustFxA.accountManager
    }
    ...
    func launchPairing(urlString: String, navigationController: UINavigationController?) {
    ...
    static func prefilledPairingURL() -> String? {
