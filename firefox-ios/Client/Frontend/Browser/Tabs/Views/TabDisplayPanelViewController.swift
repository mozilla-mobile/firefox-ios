...
final class TabDisplayPanelViewController: UIViewController,
    ...
    /// Unique identity for this panel instance. Its Redux component is added, read, and removed scoped
    /// to this identity, so a late-deallocated panel can only ever remove its own entry and can't wipe a
    /// newly-opened tab tray's state.
    private let screenIdentity = UUID()
    ...
    func unsubscribeFromRedux() {
        store.dispatch(ComponentAction(windowUUID: windowUUID,
                                       actionType: ComponentActionType.removeComponent,
                                       component: .tabsPanel,
                                       screenIdentity: screenIdentity))
    }

    private func dispatchDidLoadAction() {
        ...
        store.subscribe(self, transform: {
            return $0.select({ appState in
                return TabsPanelState(appState: appState, uuid: uuid, screenIdentity: identity)
            })
        })
    }
