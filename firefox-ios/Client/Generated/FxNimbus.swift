...
public class FxNimbus : FeatureManifestInterface {
    ...
    public func initialize(with getSdk: @escaping () -> FeaturesInterface?) {
        ...
        self.features.reduxIntegrationFeature.with(sdk: getSdk)
    ...
    public func invalidateCachedValues() {
        ...
        features.reduxIntegrationFeature.with(cachedValue: nil)
...
public class FxNimbusFeatures {
    ...
    /// This feature is for managing the roll out of the Redux integration feature
         /// 
    public lazy var reduxIntegrationFeature: FeatureHolder<ReduxIntegrationFeature> = {
        FeatureHolder(FxNimbus.shared.getSdk, featureId: "redux-integration-feature") { variables, prefs in
            ReduxIntegrationFeature(variables, prefs)
        }
    }()
...
public class HomescreenFeature: FMLObjectInterface {
    ...
    public convenience init(
        ...
        sectionsEnabled: [HomeScreenSection: Bool] = [.jumpBackIn: true, .recentExplorations: true]
    ) {
        self.init(variables: _variables, prefs: _prefs, defaults: Defaults(
...
/// This feature is for managing the roll out of the Redux integration feature
 /// 
public class ReduxIntegrationFeature: FMLObjectInterface {
    private let _variables: Variables
    private let _defaults: Defaults
    private let _prefs: UserDefaults?
    private init(variables: Variables = NilVariables.instance, prefs: UserDefaults? = nil, defaults: Defaults) {
...
extension ReduxIntegrationFeature: FMLFeatureInterface {}
/// Configuring the functionality to do with search. This will be separated into
 /// smaller sub-features in later releases.
 /// 
...
public class Shopping2023: FMLObjectInterface {
    ...
    struct Defaults {
        ...
        let productRecommendations: Bool
        let productAds: Bool
        let relay: String
        let status: Bool
    }
    ...
    public convenience init(
        ...
        config: [String: WebsiteConfig] = ["amazon": WebsiteConfig(productIdFromUrlRegex: "(?:[\\/]|$|%2F)(?<productId>[A-Z0-9]{10})(?:[\\/]|$|\\#|\\?|%2F)", validTlDs: ["com"]), "bestbuy": WebsiteConfig(productIdFromUrlRegex: "\\/(?<productId>\\d+\\.p)", validTlDs: ["com"]), "walmart": WebsiteConfig(productIdFromUrlRegex: "\\/ip\\/(?:[A-Za-z0-9-]{1,320}\\/)?(?<productId>[0-9]{3,13})", validTlDs: ["com"])],
        productRecommendations: Bool = true,
    ...
    /// If true, recommended products feature is enabled
     ///  to be shown to the user based on their preference.
     /// 
    public lazy var productRecommendations: Bool = {
        self._variables.getBool("product-recommendations") ?? _defaults.productRecommendations
    }()
...
/// The identifiers for the different images available for cards in onboarding
 /// 
public enum NimbusOnboardingImages: String, CaseIterable {
...
/// The identifiers for the different actions available for cards in onboarding
 /// 
public enum OnboardingActions: String, CaseIterable {
...
/// The identifiers for the different actions available for the insturction card
 /// in onboarding
 /// 
public enum OnboardingInstructionsPopupActions: String, CaseIterable {
...
/// The identifiers for the different types of onboarding cards.
 /// 
public enum OnboardingType: String, CaseIterable {
...
public class NimbusOnboardingCardData: FMLObjectInterface {
    ...
    /// A list of strings corresponding to targeting expressions. The card will not
     /// be shown if any expression is `true`.
     /// 
    ...
    /// A list of strings corresponding to targeting expressions. The card will
     /// be shown if all expressions `true` and if no expressions in the
     /// `disqualifiers` table are true, or if the `disqualifiers` table is
     /// empty.
     /// 
