...
class Tab: NSObject, ThemeApplicable {
    ...
    var lastKnownUrl: URL? {
        ...
        // Note: There was a check before on the legacy session data to get the last known URL
        // That mechanism is not existant anymore, so lastKnownUrl is in fact the tab.url
        return self.url
    }
