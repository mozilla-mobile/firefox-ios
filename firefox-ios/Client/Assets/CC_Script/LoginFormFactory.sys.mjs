...
// TODO(issam): eliminate circular dependencies between LoginFormFactory and Logic
...
export const LoginFormFactory = {
  ...
  createFromForm(aForm) {
    ...
    formLike.action = Logic.getFormActionOrigin(aForm);
  ...
  createFromDocumentRoot(aDocumentRoot) {
    ...
    formLike.action = Logic.getLoginOrigin(aDocumentRoot.baseURI);
  ...
  createFromField(aField) {
    if (
      ...
      (!aField.hasBeenTypePassword &&
        !Logic.isUsernameFieldType(aField)) ||
    ...
    formLike.action = Logic.getLoginOrigin(aField.ownerDocument.baseURI);
    lazy.log.debug(
      "Created non-form LoginForm for rootElement:",
