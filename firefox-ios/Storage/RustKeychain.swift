...
open class RustKeychain {
    ...
    func updateKeychainKey(_ data: String, key: String) -> OSStatus {
        return SecItemUpdate(getBaseKeychainQuery(key: key) as CFDictionary,
                             [kSecValueData: data.data(using: String.Encoding.utf8)] as CFDictionary)
    }

    func setKeychainKey(_ data: String, key: String) -> OSStatus {
        var keychainQueryDictionary = getBaseKeychainQuery(key: key)
        keychainQueryDictionary[kSecValueData as String] = data.data(using: String.Encoding.utf8)
