...
// TODO: convert this to nimbus flag
var keychainFlag = true
...
public class RustLoginEncryptionKeys {
    ...
    private func updateKeychainKey(_ value: String, key: String) {
        let status = keychain.updateKeychainKey(value, key: key)

        if status == errSecSuccess {
            return
        } else {
            let msgHeader = "Failed to update \(key) keychain key"
            var errMsg: String

            if let errString = SecCopyErrorMessageString(status, nil) {
                errMsg = "\(msgHeader): \(errString)"
            } else {
                errMsg = "\(msgHeader): Unknown error"
            }
    ...
    private func setKeychainKey(_ value: String, key: String) {
        let status = keychain.setKeychainKey(value, key: key)

        if status == errSecSuccess {
            return
        } else if status == errSecDuplicateItem {
            updateKeychainKey(value, key: key)
        } else {
            ...
            self.logger.log("Failed to set \(key) keychain key: \(detailedMsg)",
                            level: .warning,
                            category: .storage)
        }
    }
    ...
    fileprivate func createAndStoreKey() throws -> String {
        do {
            ...
            DispatchQueue.global(qos: .background).sync {
                if keychainFlag {
                    setKeychainKey(secret, key: self.loginPerFieldKeychainKey)
                    setKeychainKey(canary, key: self.canaryPhraseKey)
                } else {
...
public class RustLogins: LoginsProtocol, KeyManager {
    ...
    private func getDataFromResult(_ result: Result<String?, Error>) -> String? {
        ...
        case .failure(let err):
            ...
            self.logger.log("Failed to get keychain data: \(err)",
                            level: .warning,
                            category: .storage)
            return nil
        }
    ...
    private func getEncryptionKeyData(rustKeys: RustLoginEncryptionKeys) -> (String?, String?) {
        var keychainData: (String?, String?) = (nil, nil)

        DispatchQueue.global(qos: .background).sync {
            let key = getDataFromResult(rustKeys.keychain.queryKeychainForKey(key: rustKeys.loginPerFieldKeychainKey))
            let canary = getDataFromResult(rustKeys.keychain.queryKeychainForKey(key: rustKeys.canaryPhraseKey))
    ...
    public func getStoredKey(completion: @escaping (Result<String, NSError>) -> Void) {
        ...
        if keychainFlag {
            (key, encryptedCanaryPhrase) = getEncryptionKeyData(rustKeys: rustKeys)
        } else {
    ...
    */
    public func getKey() throws -> Data {
        let rustKeys = RustLoginEncryptionKeys()
        if keychainFlag {
            switch rustKeys.keychain.queryKeychainForKey(key: rustKeys.loginPerFieldKeychainKey) {
