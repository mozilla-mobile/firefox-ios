...
public class RustAutofill: @unchecked Sendable {
    ...
    public func verifyCreditCards(
        ...
        performDatabaseOperation { error in
            ...
            do {
                 let result = try self.storage?.scrubUndecryptableCreditCardDataForRemoteReplacement(localEncryptionKey: key)

                if result.totalScrubbedRecords > 0 {
                    GleanMetrics.CreditCardVerification.totalScrubbedRecords.add(Int32(result.totalScrubbedRecords))
                }
