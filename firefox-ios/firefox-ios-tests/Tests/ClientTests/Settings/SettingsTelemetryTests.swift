...
final class SettingsTelemetryTests: XCTestCase {
    ...
    func testAiControlToggled_recordsData() throws {
        let event = GleanMetrics.SettingsAiControls.aiControlToggled
        typealias EventExtrasType = GleanMetrics.SettingsAiControls.AiControlToggledExtra

        let expectedMetricType = type(of: event)

        let subject = createSubject()
        subject.aiControlToggled(true)
        ...
        XCTAssertEqual(mockGleanWrapper.recordEventCalled, 1)
        XCTAssertEqual(savedExtras.toggleValue, true)
        XCTAssert(resultMetricType == expectedMetricType, debugMessage.text)
    }

    func testTranslationSettingToggled_recordsData() throws {
        let event = GleanMetrics.SettingsAiControls.translationSettingToggled
        typealias EventExtrasType = GleanMetrics.SettingsAiControls.TranslationSettingToggledExtra

        let expectedMetricType = type(of: event)

        let subject = createSubject()
        subject.translationSettingToggled(false)
        ...
        XCTAssertEqual(mockGleanWrapper.recordEventCalled, 1)
        XCTAssertEqual(savedExtras.toggleValue, false)
        XCTAssert(resultMetricType == expectedMetricType, debugMessage.text)
    }

    func testPageSummariesToggled_recordsData() throws {
        let event = GleanMetrics.SettingsAiControls.pageSummariesToggled
        typealias EventExtrasType = GleanMetrics.SettingsAiControls.PageSummariesToggledExtra

        let expectedMetricType = type(of: event)

        let subject = createSubject()
        subject.pageSummariesToggled(true)
