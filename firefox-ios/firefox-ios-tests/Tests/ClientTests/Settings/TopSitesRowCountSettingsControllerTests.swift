...
@MainActor
final class TopSitesRowCountSettingsControllerTests: XCTestCase {
    ...
    func testRowSettings_reflectDefaultNumberOfRows() {
        let subject = createSubject()
        subject.settings = subject.generateSettings()
        ...
        // The default row count should be reflected by exactly one checked option.
        XCTAssertEqual(checkmarks.filter { $0.isChecked() }.count, 1)
    }

    // MARK: - Helpers
    private func createSubject() -> TopSitesRowCountSettingsController {
        let subject = TopSitesRowCountSettingsController(prefs: profile.prefs,
