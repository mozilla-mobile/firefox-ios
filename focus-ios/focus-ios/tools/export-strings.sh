...
if [ ! -d Blockzilla.xcodeproj ]; then
  echo "[E] Run this script from the project root as focus-ios/tools/export-strings.sh"
  exit 1
fi
...
sed -ri '' 's/\/tmp\/ios-localization/\/tmp\/ios-localization-\\(getpid())/g' focus-ios/tools/Localizations/Sources/LocalizationTools/tasks/ExportTask.swift
