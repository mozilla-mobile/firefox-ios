...
class RequestHandler {
    func handle(request: URLRequest, alertCallback: (UIAlertController) -> ()) -> Bool {
        ...
        switch host {
        case "maps.apple.com":
            let alert = RequestHandler.makeAlert(title: UIConstants.strings.externalLinkTitle, action: UIConstants.strings.externalLinkOpenMaps, forURL: url)
            alertCallback(alert)
            return false
        case "itunes.apple.com":
            let alert = RequestHandler.makeAlert(title: UIConstants.strings.externalLinkTitle, action: UIConstants.strings.externalLinkOpenAppStore, forURL: url)
            alertCallback(alert)
            return false
        default:
            return true
