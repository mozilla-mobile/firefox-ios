...
class HawkHelperTests: XCTestCase {
    ...
    func testSpecRequestString() {
        ...
        let expected = "hawk.1.header\n" +
            ...
            "\n" +
            "some-app-ext-data\n";
        XCTAssertEqual(HawkHelper.getRequestStringFor(req, timestampString: String(timestamp), nonce: nonce, hash: "", extra: extra).componentsSeparatedByString("\n"),
    ...
    func testSpecWithoutPayloadExample() {
        ...
        let nonce = "j4h3g2"
        let extra = "some-app-ext-data"
        let value = helper.getAuthorizationValueFor(req, at: timestamp, nonce: nonce, extra: extra)
