...
extension SQLiteHistory: BrowserHistory {
    ...
    public func removeHistory(forURL url: String) -> Success {
    ...
    private func insertSite(_ site: Site, atTime time: NSNumber, withConnection conn: SQLiteDBConnection) -> Int {
    ...
    public func getSitesByFrecency(withHistoryLimit limit: Int) -> Deferred<Maybe<Cursor<Site>>> {
        return self.getSitesByFrecency(withHistoryLimit: limit, includeIcon: true)
    }

    public func getSitesByFrecency(withHistoryLimit limit: Int, includeIcon: Bool) -> Deferred<Maybe<Cursor<Site>>> {
        // Exclude redirect domains. Bug 1194852.
        let (whereData, groupBy) = self.topSiteClauses()
        return self.getFilteredSitesByFrecency(withHistoryLimit: limit, bookmarksLimit: 0, groupClause: groupBy, whereData: whereData, includeIcon: includeIcon)
    }

    public func getTopSites(withLimit limit: Int) -> Deferred<Maybe<Cursor<Site>>> {
    ...
    public func refreshTopSitesCache() -> Success {
        ...
        return updateTopSitesCache(withLimit: cacheSize)
    }

    public func areTopSitesDirty(withLimit limit: Int) -> Deferred<Maybe<Bool>> {
        let (whereData, groupBy) = self.topSiteClauses()
        let (query, args) = self.filteredSitesByFrecencyQuery(withHistoryLimit: limit, bookmarksLimit: 0, groupClause: groupBy, whereData: whereData)
    ...
    private func updateTopSitesCache(withLimit limit : Int) -> Success {
        let (whereData, groupBy) = self.topSiteClauses()
        let (query, args) = self.filteredSitesByFrecencyQuery(withHistoryLimit: limit, bookmarksLimit: 0, groupClause: groupBy, whereData: whereData)
    ...
    public func getSitesByFrecency(withHistoryLimit limit: Int, bookmarksLimit: Int, whereURLContains filter: String) -> Deferred<Maybe<Cursor<Site>>> {
        return self.getFilteredSitesByFrecency(withHistoryLimit: limit, bookmarksLimit: bookmarksLimit, whereURLContains: filter, includeIcon: true)
    }

    public func getSitesByFrecency(withHistoryLimit limit: Int, whereURLContains filter: String) -> Deferred<Maybe<Cursor<Site>>> {
        return self.getFilteredSitesByFrecency(withHistoryLimit: limit, bookmarksLimit: 0, whereURLContains: filter, includeIcon: true)
    }

    public func getSitesByLastVisit(withLimit limit: Int) -> Deferred<Maybe<Cursor<Site>>> {
        return self.getFilteredSitesByVisitDate(withLimit: limit, whereURLContains: nil, includeIcon: true)
    }

    private class func basicHistoryColumnFactory(_ row: SDRow) -> Site {
    ...
    private class func iconColumnFactory(_ row: SDRow) -> Favicon? {
    ...
    private class func iconHistoryColumnFactory(_ row: SDRow) -> Site {
    ...
    private func computeWords(withFilter filter: String) -> [String] {
     ...
     */
    internal func computeWhereFragment(withFilter filter: String, perWordFragment: String, perWordArgs: (String) -> Args) -> (fragment: String, args: Args) {
        precondition(!filter.isEmpty)

        let words = computeWords(withFilter: filter)
        return self.computeWhereFragment(forWords: words, perWordFragment: perWordFragment, perWordArgs: perWordArgs)
    }

    internal func computeWhereFragment(forWords words: [String], perWordFragment: String, perWordArgs: (String) -> Args) -> (fragment: String, args: Args) {
    ...
    private func getFilteredSitesByVisitDate(withLimit limit: Int,
                                                      whereURLContains filter: String? = nil,
                                                      includeIcon: Bool = true) -> Deferred<Maybe<Cursor<Site>>> {
        let args: Args?
        let whereClause: String
        if let filter = filter?.stringByTrimmingCharactersInSet(CharacterSet.whitespaceCharacterSet()) where !filter.isEmpty {
            let perWordFragment = "((\(TableHistory).url LIKE ?) OR (\(TableHistory).title LIKE ?))"
            let perWordArgs: (String) -> Args = { ["%\($0)%", "%\($0)%"] }
            let (filterFragment, filterArgs) = computeWhereFragment(withFilter: filter, perWordFragment: perWordFragment, perWordArgs: perWordArgs)
    ...
    private func getFilteredSitesByFrecency(withHistoryLimit limit: Int,
                                                            bookmarksLimit: Int,
                                                            whereURLContains filter: String? = nil,
        ...
        let (query, args) = filteredSitesByFrecencyQuery(
            withHistoryLimit: limit,
    ...
    private func filteredSitesByFrecencyQuery(withHistoryLimit historyLimit: Int,
                                                              bookmarksLimit: Int,
                                                              whereURLContains filter: String? = nil,
                                                              groupClause: String = "GROUP BY historyID ",
                                                              whereData: String? = nil,
        ...
        if let filter = filter?.trimmingCharacters(in: CharacterSet.whitespaces) where !filter.isEmpty {
...
extension SQLiteHistory: Favicons {
    ...
    func getFavicons(forURL url: String) -> Deferred<Maybe<Cursor<Favicon?>>> {
    ...
    func getFavicons(forBookmarkedURL url: String) -> Deferred<Maybe<Cursor<Favicon?>>> {
        let sql =
        "SELECT " +
...
extension SQLiteHistory: SyncableHistory {
     ...
     */
    public func ensurePlace(withURL url: String, hasGUID guid: GUID) -> Success {
    ...
    public func delete(byGUID guid: GUID, deletedAt: Timestamp) -> Success {
        let args: Args = [guid]
    ...
    private func getSiteID(forGUID guid: GUID) -> Deferred<Maybe<Int>> {
        let args: Args = [guid]
    ...
    public func storeRemoteVisits(_ visits: [Visit], forGUID guid: GUID) -> Success {
        return self.getSiteID(forGUID: guid)
    ...
    private func metadata(forGUID guid: GUID) -> Deferred<Maybe<HistoryMetadata?>> {
    ...
    public func insertOrUpdatePlace(_ place: Place, modified: Timestamp) -> Deferred<Maybe<GUID>> {
        ...
        return self.ensurePlace(withURL: place.url, hasGUID: place.guid)
            >>> { self.metadata(forGUID: place.guid) >>== insertWithMetadata }
    }
