...
struct InstructionsViewControllerUX {
    ...
    static let TextFont = UIFont.systemFont(ofSize: UIFont.labelSize())
...
private func highlightLink(_ s: NSString, withColor color: UIColor) -> AttributedString {
    let start = s.range(of: "<")
    if start.location == NSNotFound {
        return AttributedString(string: s as String)
    }

    var s: NSString = s.replacingCharacters(in: start, with: "")
    let end = s.range(of: ">")
    s = s.replacingCharacters(in: end, with: "")
