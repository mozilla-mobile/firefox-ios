...
class WKReadabilityOperation: Operation,
    ...
    override func main() {
        ...
        mainQueue.async(execute: { () in
            let configProvider = DefaultWKEngineConfigurationProvider(parameters: WKWebviewParameters())
            let session = WKEngineSession(readerModeDelegate: self, configurationProvider: configProvider)
