...
fragment half4 animatedGradientFragment(VertexOutput fragmentInput [[stage_in]],
    ...
    // Animation constants
    const float kControlPointAnimationRadius = 0.25f;
    const float kBaseInfluenceRadius = 0.8f;
    const float kPulsationAmplitude = 0.1f;
    const float kPulsationOffset = 0.15f;
    const float kNoiseScale = 12.0f;
    const float kNoiseInfluence = 0.15f;
