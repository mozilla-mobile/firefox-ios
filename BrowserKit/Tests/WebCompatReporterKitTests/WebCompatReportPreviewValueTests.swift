...
final class WebCompatReportPreviewValueTests: XCTestCase {
    func testString_isQuoted() {
        XCTAssertEqual(
            WebCompatReportPreviewViewModel.PreviewValue.string("https://example.com").displayText,
            "\"https://example.com\""
        )
    }
    ...
    func testList_isBracketedAndQuoted() {
        XCTAssertEqual(
            WebCompatReportPreviewViewModel.PreviewValue.list(["en-US", "fr-FR"]).displayText,
            "[\"en-US\", \"fr-FR\"]"
        )
    }
